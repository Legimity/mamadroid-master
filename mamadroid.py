...
def parseargs():
    parser = argparse.ArgumentParser(
        description="MaMaDroid - Analyze Android Apps for Maliciousness. For optimum performance, run on a machine with more than 16G RAM. Minimum RAM requirement is 4G."
    )
    parser.add_argument(
        "-f",
        "--file",
    ...
    parser.add_argument(
        "-d",
        "--dir",
        help="The path to your Android platform directory",
        type=str,
        required=True,
    )
...
def _make_dirs(_base_dir):
    try:
        ...
        print(
            "MaMaDroid Info: One or more of the default directory already exists. Skipping directory creation..."
        )
...
def main():
    ...
    if os.path.isdir(apps.file):
        ...
        for app in glob.glob(apps.file + "/*.apk"):
            cmd = (
                "java -Xms20g -Xmx50g -XX:+UseConcMarkSweepGC Appgraph "
                + app
                + " "
                + apps.dir
            )
